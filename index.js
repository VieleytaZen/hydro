...
async function hydroInd() {
    ...
    hydro.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            const isOwner = [...owner, global.ownernomer, global.botnumber].map(v => v ? v.replace(/[^0-9]/g, '') + '@s.whatsapp.net' : '').includes(sender)

            if (!hydro.public && !kay.key.fromMe && !isOwner && chatUpdate.type === 'notify') return
